...
value (Num n) _ = n -- My code.
value (Var ident) (dict) = case Dictionary.lookup ident dict of
                            Just x -> x
                            Nothing -> error ("var=" ++ ident ++ " not in scope.") -- My code.
value (Add le re) (dict) = value le dict + value re dict -- My code.
value (Sub le re) (dict) = value le dict - value re dict -- My code.
value (Mul le re) (dict) = value le dict * value re dict -- My code.
value (Div le re) (dict) -- My code.
 | rhs == 0 = error "divide by zero"
